// Nepal ROI
var minLat = 26.347;
var minLong = 80.018;
var maxLat = 30.447;
var maxLong = 88.201;

var roi = ee.Geometry.Rectangle([minLong, minLat, maxLong, maxLat]);
Map.centerObject(roi, 6);
Map.addLayer(roi, {color: 'red'}, 'ROI');


// CHANGE YEAR HERE
var year = 2018;


// Date range: Jan 1 – May 31
var startDate = ee.Date.fromYMD(year, 1, 1);
var endDate   = ee.Date.fromYMD(year, 5, 31);


// ERA5-Land daily temperature
var tempCol = ee.ImageCollection('ECMWF/ERA5_LAND/DAILY_AGGR')
  .filterDate(startDate, endDate)
  .filterBounds(roi)
  .select('temperature_2m');

// Create 16-day windows manually
var days = ee.List.sequence(0, endDate.difference(startDate, 'day'), 16);

days.getInfo().forEach(function(d) {

  var winStart = startDate.advance(d, 'day');
  var winEnd   = winStart.advance(16, 'day');

  var temp16 = tempCol
    .filterDate(winStart, winEnd)
    .mean()
    .subtract(273.15)        // Kelvin to Celsius
    .clip(roi)
    .rename('temp_C');

  var dateStr = winStart.format('YYYYMMdd').getInfo();
  var name = 'temp16_' + year + '_' + dateStr;

  Export.image.toDrive({
    image: temp16,
    description: name,
    folder: 'GEE_Exports_16/temp',
    fileNamePrefix: name,
    region: roi,
    scale: 1000,
    crs: 'EPSG:4326',
    maxPixels: 1e13
  });
});


// Quick visual check
Map.addLayer(
  ee.Image(tempCol.first()).subtract(273.15).clip(roi),
  {min: -10, max: 35,  palette: ['0b1d51', '1565c0', '42a5f5', 'a5d6a7', 'fff59d', 'ffb74d', 'e53935']
},
  'Temp example'
);
