...
export default function VerifyPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    getVerification({ start: "2024-01-01", end: "2025-05-30" })
      .then((body) => {
    ...
    <div className="absolute inset-0 pt-14 overflow-y-auto">
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-10">
        ...
        <p className="text-sm text-muted max-w-[62ch] mt-4 leading-relaxed">
          Each morning we paint a danger map. The next day, satellites tell us
          where fire actually appeared. This page compares the two. A quiet day
          with almost no fire is shown as a dash — not a fake zero.
        </p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mt-6">
          ...
          <Card className="p-4">
            <p className="label-ui text-muted">Chance error</p>
            <p className="font-display font-extrabold text-3xl md:text-4xl tabular-nums mt-1">
              {fmt(summary.mean_brier)}
            </p>
            <p className="text-[11px] text-muted mt-1">Lower is better.</p>
          </Card>
        </div>
